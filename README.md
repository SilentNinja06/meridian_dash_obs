...
- **Directives** — a persistent to-do engine: recurring items, future scheduling, per-occurrence dismiss, and roll-and-flag for slipped items. Does not reset overnight; completions are archived under `# Completed tasks`. Each directive can also hold a collapsible checklist of sub-tasks (per-occurrence for recurring items), one muted note line, an optional **due date** (shown as a chip; past-due reads “overdue”), and a per-item toggle to draw it on the printed week planner. A **Weekly review** button opens a read-only 7-day observation summary compiled from the daily notes.
- **Today's Agenda** — up to 10 Proton Calendar share links (public ICS), today only, with an offline cache and visible fetch failures. A **NEXT / NOW** placard shows what's next and how long you're free (ticking each minute), and each calendar has a configurable swatch colour and can be toggled out of that countdown while still showing on the agenda. Dashboard-only **local events** (a **+ Event** button, tagged `LOCAL`) merge into today's agenda and feed the same countdown — useful for anything the read-only Proton feed will never carry. A **Weekly goals** button sets goals for the week (each can be sent to the Directives list, due at week's end), and a **Print week** button opens a printable week-at-a-glance planner (US Letter, landscape): the week's goals at the top, each day's events colour-coded by their source calendar (with a legend), any directives opted onto the planner, plus ruled space to write in. On the desktop it prints directly; on mobile a **Share / Print** button hands the planner to the OS share sheet (AirPrint, Save to Files, or open in a browser).
...
- **Set weekly goals** — opens the current week's goals.
